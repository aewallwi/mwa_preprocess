...
mwa_preprocess_dir = os.path.dirname(os.path.realpath(__file__))
...
def _get_git_output(args, capture_stderr=False):
    ...
    argv = ['git', '-C', mwa_preprocess_dir] + args
...
def _get_gitinfo_file(git_file=None):
    ...
    if git_file is None:
        git_file = os.path.join(mwa_preprocess_dir, 'GIT_INFO')
...
def construct_version_info():
    version_file = os.path.join(mwa_preprocess_dir, 'VERSION')
